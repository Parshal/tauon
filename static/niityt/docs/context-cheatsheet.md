...
- **State** – `static/niityt/state.js`: owns grid buffers (`grid`, `cellColors`, `flowers`), timers, toolbelt, fertilizer, and render payload assembly.
...
- Flowers: `FLOWER_NONE = 0`, `FLOWER_FERTILIZER = 1`, `FLOWER_VARIANTS = [2…7]`.
- Caps: `FERTILIZER_MAX = 80`, pickup glow window `RECENT_PICKUP_WINDOW = 1.5` seconds.

## State Flow Highlights (`state.js`)
1. **Constructor**: allocates buffers (`Uint8Array`), sets control band height (`height * 0.08`), seeds energy, timers, toolbelt array, fertilizer counter, and invokes `generateFlowerLayer()`.
...
3. **`spread()`**: chooses random claimed source, attempts propagation; empty neighbors inherit color via `inheritColorFromSource()` and call `onCellClaimed()` which pushes index, harvests a flower, stamps `cellColors`.
4. **Placement**: `placeControl(x,y)` enforces control band (`y >= height - controlBandHeight`), energy cost, emptiness, then seeds `grid[idx] = 255` and consumes the active toolbelt flower.
5. **Flower harvesting**: `harvestFlower(idx)` reads `flowers[idx]`; fertilizer increments `this.fertilizer`, colors go through `collectFlower()` unlock logic, then the flower cell is reset.
...
- Textures: `u_grid` (R8 growth values) + `u_cellColors` (flower IDs scaled by 1/255).
...
- Fragment helpers: `groundColor()`, `playerColor()`, `flowerPalette()`, `renderBand()`, `renderPointer()`, `renderToolbeltRail()` (handles slot fill, stack ticks, active glow, fertilizer column, pickup glow).
