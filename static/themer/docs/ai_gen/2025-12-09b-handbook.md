...
6. Treat this handbook as the single touchpoint—refresh sections here rather than spawning new docs.
7. Keep the invocation prompt pristine and the primer human-friendly; update their prose when behavior or tone shifts.
...
-1. Prefer not to use much commands when creating a new version as they suck in windsurf.
0. **Archive before editing.** Write a new file under `docs/ai_gen/` (e.g., `YYYY-MM-DDx-handbook.md`) that captures the current handbook before touching this doc; never overwrite existing snapshots, and only use inline appendices (`…-09b`) for additional passes within the same day.
...
5. Re-read the doc to ensure alignment with inspected code, then summarize in chat exactly which sections changed and whether TODOs moved.
...
- Keep this primer approachable—when adding advanced tweaks, link or summarize rather than dumping raw diffs.
